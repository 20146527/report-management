 ...
 */
var binaryToSteno = {};
$.getJSON('../../file/keycode/binaryToSteno.json', function (data) {
    binaryToSteno = data;
});
...
$.getJSON('../../file/keycode/keyCodeToQwerty.json', function (data) {
 ...
 */
var keyCodeToSteno = {};
$.getJSON('../../file/keycode/keyCodeToSteno.json', function (data) {
    keyCodeToSteno = data;
});
 ...
 */
var stenoKeyNumbers = {};
$.getJSON('../../file/keycode/stenoKeyNumbers.json', function (data) {
        ...
        if (isSteno) {
            ...
            if ($.isEmptyObject(downKeys)) { // if no more keys are being pressed down, this is the end of the stroke.
                ...
                for (i = 0; i < words.length; i++) {
                    let s = words[i].toEnglish();
                    if ($.isNumeric(s)) {
                        if (parseInt(s) < 0)
                            translatedString += -parseInt(s);
                        else
                            translatedString += s;
                    } else if (!$.isNumeric(s) && i > 0 && $.isNumeric(words[i - 1].toEnglish())) {
                        translatedString += ' ' + s + ' ';
                    } else {
                        translatedString += s + ' ';
                    }
