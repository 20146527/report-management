...
let onChangeDataInput = (id) => {
    ...
    if (check(value)) {
        ...
        if (bool) {
            let key_code = Object.keys(content);
...
let onchangeInputWord = (id) => {
    let notification = $("#label-value-" + id);
    let value = $("#value-" + id).val();
    let key_code = Object.values(content);
    if ($.inArray(value, key_code) >= 0) {
...
function onClickUpdateDict() {
    ...
    if (!event_submit_button(number_key)) {
        ...
        json = "{" + json + "}";
        $.ajax({
            url: "/manager-person-dict-steno.html",
            type: "post",
            data: {
                type: "add-new-person-dict",
                dataPersonDict: json
            },
            success: function () {
...
function redirectFunction(type, elem) {
    let name = $(elem).closest("tr").find(".index").attr("id");
    let data = $.base64.encode(name);
    window.location.href = "/manager-person-dict-steno.html?type=" + type + "&index=" + data;
}

function fnDeleteDict(type, elem) {
    let name = $(elem).closest("tr").find(".index").attr("id");
    let data = $.base64.encode(name);
    swal({
            title: "Thông báo ?",
...
function editDict(elem) {
    let name = $(elem).closest("tr").find(".index").attr("id");
    let data = $.base64.encode(name);
    let code_steno = $(elem).closest("tr").find(".code-steno").text();
    let word = $(elem).closest("tr").find(".word").text();
    $("#steno-code").val(code_steno);
    $(".modal-code").text(code_steno);
    $("#steno-value").val(word);
    $(".modal-steno-value").text(word);
    $(".code-error-steno").css({"display": "none"});
    $(".value-error-steno").css({"display": "none"});
    $(".modal-value").text(data);
    $('#edit-person-dict').modal('show');
}
